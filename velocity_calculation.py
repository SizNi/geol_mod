...
def velocity(n_x, n_y, d_x, d_y, modelling_matrix, well_matrix):
    ...
    for i in range(well_count):
        x_1[i] = np.sum((d_x[: n_x_skv[i] - 1] + d_x[1 : n_x_skv[i]]) / 2)
        y_1[i] = np.sum((d_y[: n_y_skv[i] - 1] + d_y[1 : n_y_skv[i]]) / 2)
    ...
    for i in range(1, n_y):
        ...
        for j in range(n_x):
            for k in range(well_count):
                ...
                y_h = y_2 - y_1[k]
                x_x = x_2 - x_1[k]
                modelling_matrix[j, i].v_x -= a_1[k] * (np.arctan(y_b / x_x) - np.arctan(y_h / x_x)) / d_y[i]
            if j < n_x - 1:
                x_2 += d_x[j + 1]
        y_2 = y_3
    ...
    for i in range(1, n_x):
        ...
        for j in range(n_y):
            for k in range(well_count):
                ...
                x_l = x_2 - x_1[k]
                y_y = y_2 - y_1[k]
                modelling_matrix[i, j].v_y -= a_1[k] * (np.arctan(x_p / y_y) - np.arctan(x_l / y_y)) / d_x[i]
            if j < n_y - 1:
                y_2 += d_y[j + 1]
        x_2 = x_3
